...
class BotSettingsTab(ttk.Frame):
    ...
    def save_bot_settings(self):
        ...
        try:
            ...
            save_bot_settings(token, chat_id)  # обновляет БД и глобальные переменные TOKEN и SCHEDULED_CHAT_ID
            messagebox.showinfo("Успех", "Настройки бота обновлены и применены!")
            global BOT_APPLICATION, BOT_LOOP
            if BOT_APPLICATION is not None and BOT_LOOP is not None:
                try:
                    # Останавливаем запущенного бота через сохранённый event loop
                    future = asyncio.run_coroutine_threadsafe(BOT_APPLICATION.stop(), BOT_LOOP)
                    future.result(timeout=10)
                except Exception as e:
                    print(f"Ошибка при остановке бота: {e}")
            start_telegram_bot_thread()  # запускаем новый поток с обновлёнными настройками
        except ValueError:
...
async def bot_main():
    global BOT_APPLICATION, BOT_LOOP
    BOT_LOOP = asyncio.get_running_loop()  # сохраняем текущий event loop
    # Если токен невалидный, не создаём приложение
    if TOKEN in ("0", 0, "", "Введите токен"):
        print("Невалидный токен. Бот не запускается.")
        return
    BOT_APPLICATION = Application.builder().token(TOKEN).build()
    BOT_APPLICATION.add_handler(MessageHandler(filters.StatusUpdate.NEW_CHAT_MEMBERS, welcome_new_member))
    BOT_APPLICATION.add_handler(MessageHandler(filters.TEXT, track_messages))
    BOT_APPLICATION.add_handler(MessageHandler(filters.TEXT & filters.Regex('^/toggle_bot$'), toggle_bot))
    moscow_tz = pytz.timezone("Europe/Moscow")
    scheduled_time = datetime.time(hour=18, minute=0, second=0, tzinfo=moscow_tz)
    BOT_APPLICATION.job_queue.run_daily(scheduled_recipe, time=scheduled_time)
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.create_widgets()
        start_telegram_bot_thread()  # запускаем бот сразу при старте приложения
