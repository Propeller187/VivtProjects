...
def get_product_for_task_order(task):
    # Извлекаем category_id и subcategory_id по именам
    category_id, subcategory_id = get_category_and_subcategory_ids(task['category'], task['subcategory'])

    conn = get_db_connection()
    cursor = conn.cursor()

    # Получаем все товары для данной задачи из таблицы parsed_data
    cursor.execute("""
        SELECT * FROM parsed_data 
        WHERE category_id = %s AND subcategory_id = %s
    ...
    # Проверяем, все ли order_index равны
    order_indexes = [p['order_index'] for p in products]
    if len(set(order_indexes)) == 1:
        # Если все равны, выбираем товар с минимальным id
        chosen_product = min(products, key=lambda p: p['id'])
    else:
        # Иначе выбираем товар с минимальным order_index
        chosen_product = min(products, key=lambda p: p['order_index'])

    # Вычисляем новое значение order_index для выбранного товара
    new_order = chosen_product['order_index'] + 1

    # Обновляем order_index выбранного товара
    cursor.execute("""
        UPDATE parsed_data 
        SET order_index = %s 
        WHERE id = %s
...
def get_product_for_task_random(task):
    ...
    # Выбираем товар с минимальным значением status; если несколько, выбираем случайным образом среди них
    cursor.execute("""
        SELECT * FROM parsed_data 
        WHERE category_id = %s AND subcategory_id = %s
...
class TasksTab(ttk.Frame):
    ...
    def refresh_tasks(self):
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM bot_tasks ORDER BY id ASC")  # сортировка по порядку
    ...
    try:
        ...
        for row in parsed_data:
            cursor.execute(insert_sql, (
                ...
                0,  # status по умолчанию
                0   # order_index по умолчанию
            ))
            inserted_count += 1
        conn.commit()
    except Exception as e:
...
def run_module_worker(q, file_path, cat_name, subcat_name, use_subprocess=True):
    ...
    Она может импортировать модуль или выполнить скрипт через subprocess, затем
    выполняет функцию parsing() (или берёт данные из переменной data) и сохраняет данные,
    а затем кладёт накопленный вывод в очередь.

    :param q: Очередь для отправки результатов
    :param file_path: Путь к файлу модуля или скрипта
    :param cat_name: Название категории
    :param subcat_name: Название подкатегории
    :param use_subprocess: Флаг, указывающий, использовать ли subprocess (True — для subprocess, False — для импорта модуля)
    """
    output_capture = io.StringIO()
    with contextlib.redirect_stdout(output_capture):
        try:
            if use_subprocess:
                # Выполнение через subprocess
                result = subprocess.run([sys.executable, file_path], capture_output=True, text=True)
                output_capture.write(result.stdout)
            else:
                # Выполнение через динамический импорт модуля
...
class ModulesTab(ttk.Frame):
    ...
    def run_selected_module(self):
        if self.current_process is not None:
            messagebox.showerror("Ошибка", f"Модуль '{self.active_module_name}' уже запущен. Остановите его перед запуском другого.")
            return
        selection = self.modules_listbox.curselection()
        if not selection:
        ...
        if index < len(modules):
            ...
            self.start_process(module_record['file_path'], module_record['category'], module_record['subcategory'], module_record['module_name'])

    def start_process(self, file_path, cat_name, subcat_name, module_name, use_subprocess=True):
        ...
        self.refresh_modules_list()  # Обновляем список для выделения запущенного модуля
