...
# Работа с файлами, JSON, потоками и модулями
import json
import os
import threading
...
# Работа с Telegram и асинхронностью
import nest_asyncio
from telegram import Bot, Update
from telegram.ext import Application, MessageHandler, filters, CallbackContext
from telegram.error import ChatMigrated

# Запуск внешних процессов и эмодзи
import subprocess
import emoji

DB_CONFIG = None  # Конфигурация для подключения к базе данных
BOT_PROCESS = None  # Процесс, в котором будет работать телеграм-бот
BOT_STATUS = False  # Состояние бота (запущен/остановлен)
TOKEN = ""  # Текущий токен бота
SCHEDULED_CHAT_ID = 0  # ID чата, куда бот будет отправлять сообщения
is_db_working = 1  # Статус соединения с БД (1 - подключение есть, 0 - ошибка)
...
# Позволяет использовать вложенные asyncio циклы, например в Tkinter
nest_asyncio.apply()
...
def get_db_connection():
    """Устанавливает соединение с базой данных по текущей конфигурации."""
...
# ⛔ Останавливает работающего Telegram-бота:
# - Асинхронно завершает работу приложения и его event loop.
# - Обнуляет глобальные переменные, связанные с ботом.
# - Делает небольшую задержку, чтобы корректно завершились фоновые процессы (например, getUpdates).
...
class CategoriesTab(ttk.Frame):
    def __init__(self, master, modules_tab, tasks_tab, *args, **kwargs):
        super().__init__(master, *args, **kwargs)
        self.modules_tab = modules_tab  # Ссылка на ModulesTab
        self.tasks_tab = tasks_tab  # Сохраняем ссылку на TasksTab
    ...
    def delete_category(self):
        ...
        cursor.execute("DELETE FROM bot_tasks WHERE category = %s", (category_name,))
        ...
        self.tasks_tab.refresh_tasks()  # Обновляем список задач
    ...
    def delete_subcategory(self):
        ...
        # Удаляем задачи с этой категорией и подкатегорией
        cursor.execute("DELETE FROM bot_tasks WHERE category = %s AND subcategory = %s",
                       (self.selected_category, sub_name))
...
class App(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.tasks_tab = TasksTab(self.notebook)  # Создаём TasksTab раньше!
        self.modules_tab = ModulesTab(self.notebook)
        self.categories_tab = CategoriesTab(self, self.modules_tab, self.tasks_tab)  # Теперь tasks_tab существует
